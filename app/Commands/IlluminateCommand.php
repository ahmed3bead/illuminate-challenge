...
class IlluminateCommand extends Command
{
    ...
    private function showStage(string $token): int
    {
        ...
        try {
            ...
            $_ = $connector->verify();
            file_put_contents('/tmp/illuminate_debug.txt', get_class($e) . ': ' . $e->getMessage() . "\n" . $e->getTraceAsString());
            if ($e->getPrevious()) {
                file_put_contents('/tmp/illuminate_debug.txt', 'Caused by: ' . get_class($e->getPrevious()) . ': ' . $e->getPrevious()->getMessage(), FILE_APPEND);
            }
